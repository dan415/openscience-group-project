...
class Affiliation:
    ...
    def __init__(self, name=None, country=None, established=None, website=None, acknowledged_by=[]):
        self.name = name
    ...
    def get_wikidata_info(wd_item_id):
        ...
        results = wdi_core.WDItemEngine.execute_sparql_query(query)
        website = results["results"]["bindings"][0]["website"]["value"] if results["results"]["bindings"][0]["website"] else None
        established = results["results"]["bindings"][0]["established"]["value"] if results["results"]["bindings"][0].get("established") else None
...
class Journal:
    ...
    def __init__(self, name=None, country=None, established=None, description=None, publishes=None):
        ...
        if name:
            ...
            if wd_item_id:
                ...
                self.established = journal_info.get("established")
    ...
    def get_wikidata_info(wd_item_id):
        ...
        results = wdi_core.WDItemEngine.execute_sparql_query(query)
        country_of_origin = results["results"]["bindings"][0]["country_of_origin"]["value"] if results["results"]["bindings"][0]["country_of_origin"] else None
        description = results["results"]["bindings"][0]["description"]["value"] if results["results"]["bindings"][0].get("description") else None
        established = results["results"]["bindings"][0]["established"]["value"] if results["results"]["bindings"][0].get("established") else None
